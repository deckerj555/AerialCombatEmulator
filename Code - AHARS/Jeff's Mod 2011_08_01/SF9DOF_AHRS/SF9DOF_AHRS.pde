...
/* Hardware version - v13
 ...
 Arduino IDE : Select board  "Arduino Pro or Pro Mini w/ATmega328" @ 8Mhz!" JPD 26OCt2011
 */
...
#define WRITE_HEX 1     //will send the euler angles in hex
...
float pitch;
float yaw;
//unsigned Euler angles hex arrays
unsigned char roll_array[4];
unsigned char pitch_array[4];
unsigned char yaw_array[4];
...
{ 
  ...
  // Initialze ADC readings and g_gpsBuffers
  Read_adc_raw();
  delay(20);
...
unsigned char g_gpsBuffer[100];
int g_gpsHeaderFound = 0;
int g_gpsTailIndex = 0;
...
void loop() //Main Loop
{
  ...
  // Since we poll the IMU at 50Hz this check makes sure that we don't start the GPS
  // stuff more than 2ms before the IMU needs time.  We're assuming that the GPS
  // check takes no more than 2ms.
  if((millis() - timer) <= 18)
  {
    checkForGPSData();
  }
...
void checkForGPSData()
{
  static unsigned char header[] = {0xA0, 0xA1, 0x00, 0x3B, 0xA8};
  static unsigned char footer[] = {0x0d, 0x0a};

  if(Serial.available())
  {
    while(Serial.available() && g_gpsTailIndex < sizeof(g_gpsBuffer))
    {
      g_gpsBuffer[g_gpsTailIndex] = Serial.read();
      g_gpsTailIndex++;
    }

    if (g_gpsHeaderFound == 0)
    {
      for (int i = 0; i < g_gpsTailIndex; i++)
      {
        if (i <= g_gpsTailIndex - sizeof(header))
        {
          if (g_gpsBuffer[i] == header[0] && g_gpsBuffer[i+4] == header[4])
          { // See if we're lucky and have a whole header, if so, declare full header and go to next step.
            if (i > 0) // keeps you from copying if you're lucky enough to have the header start at the start of g_gpsBuffer
            {
              g_gpsTailIndex = g_gpsTailIndex - i;
              memmove(g_gpsBuffer, &g_gpsBuffer[i], g_gpsTailIndex);
            }
        ...
        { // If not lucky copy remainder to the begining, then break to go get more bytesRead
          g_gpsTailIndex = g_gpsTailIndex - i;
          memmove(g_gpsBuffer, &g_gpsBuffer[i], g_gpsTailIndex);
          break;
        }
    ...
    if (g_gpsHeaderFound == 1 && g_gpsTailIndex > 65)
    {
      if (g_gpsBuffer[64] == footer[0] && g_gpsBuffer[65] == footer[1])
      { // If you have a full header, and enough data to expect a full line, assign the line
        for(int j = 0; j <=65; j++)
        {
          Serial.write(g_gpsBuffer[j]);
        }
      }
      g_gpsTailIndex = g_gpsTailIndex - 65;
      memmove(g_gpsBuffer, &g_gpsBuffer[65], g_gpsTailIndex);
      g_gpsHeaderFound = 0;
    }  
  }
}
